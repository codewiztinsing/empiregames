// Fixed 5x5 Bingo Card Generator
// This ensures consistent card generation across all components
...
export function generateFixedCard(cardNumber) {
...
export function generateMultipleFixedCards(count) {
  const cards = [];
...
export function cardHasNumber(card, number) {
  for (let row of card) {
    for (let cell of row) {
      if (cell === number) {
        return true;
...
export function markCardNumber(card, number) {
...
export function checkBingoPatterns(card) {
  const patterns = {
    row: false,
    column: false,
    diagonal: false,
