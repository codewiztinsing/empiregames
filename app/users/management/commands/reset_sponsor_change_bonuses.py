...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--reset',
            action='store_true',
            help='Reset all sponsor change bonuses',
        )
        parser.add_argument(
            '--force',
            action='store_true',
    ...
    def show_status(self):
        """Show current status of sponsor change bonuses"""
        self.stdout.write(
            self.style.SUCCESS("=" * 60)
        )
        self.stdout.write(
            self.style.SUCCESS("CURRENT STATUS OF SPONSOR CHANGE BONUSES")
        )
        self.stdout.write(
            self.style.SUCCESS("=" * 60)
        )
        ...
        self.stdout.write(f"Total sponsor change bonuses: {total_bonuses}")
        ...
        if total_bonuses > 0:
            ...
            total_amount = sponsor_change_bonuses.aggregate(
    ...
    def reset_bonuses(self, force=False):
        """Reset all sponsor change bonuses"""
        self.stdout.write(
            self.style.SUCCESS("=" * 60)
        )
        self.stdout.write(
            self.style.SUCCESS("REFERRAL BONUS RESET SCRIPT")
        )
        self.stdout.write(
            self.style.SUCCESS("=" * 60)
        )
        ...
        try:
            ...
            self.stdout.write(f"Found {total_bonuses} ReferralBonus records with type 'sponsor_change'")

            if total_bonuses == 0:
                self.stdout.write(
                    self.style.WARNING("No sponsor change bonuses found. Nothing to reset.")
                )
                return
            ...
            with transaction.atomic():
                ...
                updated_users = User.objects.filter(
                    sponsor_change_bonus_claimed=True
                ).update(sponsor_change_bonus_claimed=False)
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Reset sponsor_change_bonus_claimed flag for {updated_users} users")
                )
            ...
            self.stdout.write(f"\nVerification:")
            self.stdout.write(f"• Remaining sponsor change bonuses: {remaining_bonuses}")
