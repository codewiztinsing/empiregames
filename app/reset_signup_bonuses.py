#!/usr/bin/env python
"""
Script to reset all ReferralBonus records of type 'signup' (Signup Bonus 10 birr)

This script will:
1. Find all ReferralBonus records with bonus_type='signup'
2. Delete them from the database
3. Reset the signup_bonus_claimed field for all users
4. Provide a summary of actions taken

Usage:
    python reset_signup_bonuses.py
...
def reset_signup_bonuses():
    """
    Reset all signup bonuses and user flags
    """
    print("=" * 60)
    print("SIGNUP BONUS RESET SCRIPT")
    print("=" * 60)
    print()
    ...
    try:
        ...
        print(f"Found {total_bonuses} ReferralBonus records with type 'signup'")
        
        if total_bonuses == 0:
            print("No signup bonuses found. Nothing to reset.")
            return
        ...
        with transaction.atomic():
            ...
            updated_users = User.objects.filter(signup_bonus_claimed=True).update(signup_bonus_claimed=False)
            print(f"✓ Reset signup_bonus_claimed flag for {updated_users} users")
        ...
        print(f"\nVerification:")
        print(f"• Remaining signup bonuses: {remaining_bonuses}")
...
def show_current_status():
    """
    Show current status of signup bonuses without making changes
    """
    print("=" * 60)
    print("CURRENT STATUS OF SIGNUP BONUSES")
    print("=" * 60)
    ...
    print(f"Total signup bonuses: {total_bonuses}")
    ...
    if total_bonuses > 0:
        ...
        print(f"\nTotal amount: {signup_bonuses.aggregate(total=models.Sum('bonus_amount'))['total'] or 0} ETB")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Reset signup bonuses')
    parser.add_argument('--status', action='store_true', help='Show current status without making changes')
    parser.add_argument('--reset', action='store_true', help='Reset all signup bonuses')
    ...
    if args.status:
        ...
        reset_signup_bonuses()
    else:
        print("Signup Bonus Reset Script")
        print("=" * 40)
        print("Usage:")
        print("  python reset_signup_bonuses.py --status  # Show current status")
        print("  python reset_signup_bonuses.py --reset   # Reset all signup bonuses")
        print()
        print("This script will reset all ReferralBonus records of type 'signup'")
        print("and reset the signup_bonus_claimed flag for all users.")
