...
def handle_deposit_success(tx_ref):
    ...
    if chapa_session:
        ...
        if user:
            ...
            if wallet:
                # Credit exact deposit amount (no bonus)
                deposit_amount = float(chapa_session.amount)
                wallet.balance += deposit_amount
                wallet.save()
                transaction = Transaction.objects.create(user=user,amount=deposit_amount,type="DEPOSIT",status="success",reference=chapa_session.tx_ref)
